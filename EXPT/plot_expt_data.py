...
def estimate_coef(x, y): 
    ...
    print('SS_xx =',np.sqrt(SS_xx),' SS_xy =',np.sqrt(SS_xy)) 
...
for i in [0,1]:
     ...
     print('Vset=',Vset)
     print('Iset=',Iset)
     ax.scatter(Vset, Iset, marker=markers[i], color=colors[i], label=geo[i])
     ...
     print('slope =', slope) 
     y_pred = b0 + b1*Vset 
     plt.plot(Vset, y_pred, color = colors[i], label='slope ={}'.format(slope))
