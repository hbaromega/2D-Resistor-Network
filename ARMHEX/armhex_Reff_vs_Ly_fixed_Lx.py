...
def main():
  ...
  for Lx in L_list:
    ...
    f0=open('Reff_vs_Ly_fixed_Lx{}_armhex.dat'.format(Lx), 'w')
    f1=open('Rratio_vs_Ly_fixed_Lx{}_armhex.dat'.format(Lx), 'w')
    print('Output files=',f0, f1)
    for Ly in range(2,201):  # Breaks down for Ly=1, hence starting from Ly=2
         ...
         print(Ly,Reff*Ly/float(R*Lx),1/Ly, file=f1)
    f0.close() 
    f1.close() 
  ...
  import plot_armhex_Rratio_vs_Ly_fixed_Lx 
...
def Effective_resistance(V0,R,Lx,Ly):
  ...
  ## Top border points (i=0 to Lx-1, j=Ly, k=1) : S-type
  for i in range(Lx):
    if (i%2==0):
