...
xlabel="$1/L_y$"
ylabel="ln $r$"
#ylabel="$R_{\mathrm{eff}}$ $L_y$/$(R L_x)$"
...
import re
print('here')
path = 'Rratio_vs_Ly_fixed_Lx*_armhex.dat'
List=[]
...
for Lx in List:
     ...
     dir='.' 
     #dir='../DATAFILES'
     datafile = '{}/Rratio_vs_Ly_fixed_Lx{}_armhex.dat'.format(dir,Lx)
     data = np.loadtxt(datafile)
     print('datafile =',datafile)
     ax.plot(data[:,2], np.log(data[:,1]), linewidth=2.0*(1+i/4), linestyle=lines[i], color=colors[i], label='$L_x$ = {}'.format(Lx))
...
# Annotation 
xannote=0.35; yannote=1.07 # absolute coordinates
...
# SAVE AND SHOW PLOT
# -----------------------------------
fname='lnRratio_vs_Lyinv_fixed_Lx_armhex'
